...
export default async function MoneyTransferPartnerPage({ params }: Props) {
  const { partner: slug } = await params
  const partner = partners.moneyTransfer.find((item) => item.slug === slug)
          ...
          <div className="mt-6 sm:mt-8 flex flex-col md:flex-row gap-6 sm:gap-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              {partner.countries && partner.countries.length > 0 && (
                <div className="mt-6">
                  <h3 className="font-semibold text-sm sm:text-base">Supported Countries</h3>
                ...
                {outboundLink ? (
                  <Button asChild className="w-full sm:w-auto">
                    <a href={outboundLink} target="_blank" rel="noopener noreferrer">
                      Transfer via {partner.name}
                      <ExternalLink className="ml-2 h-4 w-4" />
                    </a>
                  </Button>
                ) : (
                  <Button asChild className="w-full sm:w-auto">
                    <Link href="/services/money-transfer#enquiry">Enquire Now</Link>
                  </Button>
                )}
                <Button asChild variant="outline" className="w-full sm:w-auto">
                  <Link href="/services/money-transfer#enquiry">Get Assistance</Link>
                </Button>
              </div>
            </div>
          </div>
